...
pub const Iter = struct {
    ...
    pub fn init(list: List) !Iter {
        const maybe_iter: ?*c.PyObject = c.PyObject_GetIter(list.object);
...
pub fn length(list: List) usize {
    return @intCast(c.PyObject_Length(list.object));
}
...
pub fn sliceOfAny(list: List) [][*c]c.PyObject {
    const py_list: *c.PyListObject = @ptrCast(list.object);
...
pub fn getAny(list: List, index: usize) *c.PyObject {
    ...
    if (c.PyList_GetItem(list.object, @intCast(index))) |obj| {
        return obj;
    } else {
...
pub fn setAny(list: List, index: usize, value: *c.PyObject) void {
    ...
    if (c.PyList_SetItem(list.object, @intCast(index), value) == 0) {} else {
        unreachable;
    }
}
